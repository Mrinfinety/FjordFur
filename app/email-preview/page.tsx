...
function buildHtml(en: boolean) {
  const navn = 'Ola Nordmann';
  const ordreNr = 'FF-ABC123456789';
  const totalNok = '449';
  const items = [
      ...
      <div style="padding:32px">
        ...
        <div style="background:#f7f7f5;border-radius:6px;padding:16px 20px;margin-bottom:24px">
          <p style="font-size:14px;color:#444;margin:0;line-height:1.7">
            ${en
              ? 'Your package will be shipped directly from our warehouse and delivered within <strong>2–3 weeks</strong>.'
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', background: '#e8e8e4', minHeight: '100vh', padding: '24px' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
          ...
          <button
            onClick={() => setLang('no')}
            style={{ padding: '6px 16px', borderRadius: '4px', border: 'none', cursor: 'pointer', background: lang === 'no' ? '#1a1a18' : '#ccc', color: lang === 'no' ? '#fff' : '#333', fontWeight: 600 }}
          >
            Norsk
          </button>
          <button
            onClick={() => setLang('en')}
            style={{ padding: '6px 16px', borderRadius: '4px', border: 'none', cursor: 'pointer', background: lang === 'en' ? '#1a1a18' : '#ccc', color: lang === 'en' ? '#fff' : '#333', fontWeight: 600 }}
          >
            English
          </button>
        </div>
